...
The `daily-indexing` skill reads this file, picks the next pending pages, submits
them, and updates the state.

## State

- **Priority** — 1 (highest) to 5 (lowest). Pick the lowest-numbered pending row first.
- **GSC** — ISO date the URL was submitted via GSC URL Inspection → Request Indexing. Empty means pending.
- **Bing** — ISO date the URL was submitted via Bing Webmaster → URL Submission. Empty means pending.

## Daily caps

- **GSC**: ~10-20 URLs/day per property (Google throttles silently beyond this — don't push).
- **Bing**: 10,000/day quota — can submit all pending URLs in one batch on first run.

## Priority buckets

| Priority | What | Rationale |
|---|---|---|
| 1 | Homepage, /languages, `/es/*` high-demand pages | Bing's 53.7K Spanish impressions (técnicas de respiración family) — biggest untapped demand |
| 2 | `/es/*` remainder + buteyko/tummo/ujjayi/physiological-sigh/wim-hof in all locales | AI-Overview-affected queries; needs index + strong E-E-A-T signals |
